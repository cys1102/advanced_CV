...
while True:
    ...
    if len(lmList) != 0:
        ...
        volBar = np.interp(length, [50, 140], [400, 150])
        volPer = np.interp(length, [50, 140], [0, 100])

        print(int(length), vol)
        # volume.SetMastVolumeLevel(vol, None)
    ...
    cv.putText(img, f"{int(volPer)} %", (40, 450), cv.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)

    cTime = time.time()
    fps = 1 / (cTime - pTime)
    pTime = cTime

    cv.putText(img, f"FPS: {int(fps)}", (20, 30), cv.FONT_HERSHEY_PLAIN, 1, (255, 0, 0), 2)
    cv.imshow("Image", img)
    cv.waitKey(1)
